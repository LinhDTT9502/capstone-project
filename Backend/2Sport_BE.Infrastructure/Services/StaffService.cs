...
namespace _2Sport_BE.Infrastructure.Services
{
    public interface IStaffService
    {
        ...
        Task<ResponseDTO<StaffVM>> GetStaffDetailsAsync(int staffId);
    ...
    public class StaffService : IStaffService
    {
        ...
        public async Task<ResponseDTO<StaffVM>> GetStaffDetailsAsync(int staffId)
        {
            var response = new ResponseDTO<StaffVM>();
            try
            {
        ...
        public async Task<ResponseDTO<List<StaffVM>>> GetStaffsByBranchId(int branchId)
        {
            ...
            try
            {
                var query = await _unitOfWork.StaffRepository.GetAndIncludeAsync(s => s.BranchId == branchId, new string[] {"User"});
                if (query.Count() > 0)
                {
        ...
        public async Task<ResponseDTO<StaffVM>> CreateStaffAsync(StaffCM staffCM)
        {
            ...
            try
            {
                ...
                if (user.RoleId != (int)UserRole.Staff)
                {
                    response.IsSuccess = false;
