...
const RefundRequestForm = ({ orderDetail, setRefundReload }) => {
  ...
  const [reason, setReason] = useState(""); // Lý do hoàn tiền
  const [notes, setNotes] = useState(""); // Ghi chú bổ sung
  const [isAgreementAccepted, setIsAgreementAccepted] = useState(false); // Checkbox đồng ý
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      orderCode: orderDetail.saleOrderCode,
      orderType: 1,
      reason,
      notes,
      isAgreementAccepted,
  ...
  const reasons = [
    { name: "Đơn đã hủy, cần hoàn tiền đã thanh toán" },
    { name: "Khác" },
  ];
          ...
          <div className="bg-white p-8 rounded-md shadow-lg w-full max-w-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <Input
                  type="text"
                  value={orderDetail?.saleOrderCode || ""}
                  readOnly
